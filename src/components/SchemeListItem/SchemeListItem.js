...
function SchemeListItem({ drawing, onClickItem }) {
  ...
  return (
    <div className={`scheme-list-item ${drawing.status === 'current' ? 'scheme-list-item_current' : ''}`}>
      <button
        className={`scheme-list-item__drawing ${drawing.status === 'current' ? 'scheme-list-item__drawing_current' : ''}`}
        type="button"
        name="choose-drawing"
        disabled={drawing.status === 'current'}
      ...
      <button
        className={`scheme-list-item__action ${drawing.status === 'current' ? 'scheme-list-item__action_current' : ''}`}
        type="button"
        name="edit-delete-drawing"
        onClick={drawing.status === 'current' ? handleClickEdit : handleClickDelete}
      >
